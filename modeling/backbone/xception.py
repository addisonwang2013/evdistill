...
class AlignedXception(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.block1(x)
        output_2 = x
        output_encoder.append(output_2)
        # add relu here
        x = self.relu(x)
        low_level_feat = x
        x = self.block2(x)
        output_3 = x
        output_encoder.append(output_3)

        x = self.block3(x)
        output_4 = x
        output_encoder.append(output_4)
        ...
        x = self.bn3(x)
        output_13= x
        output_encoder.append(output_13)
        x = self.relu(x)
        ...
        x = self.bn5(x)
        output_14 =x
        output_encoder.append(output_14)
        x = self.relu(x)
